...
| **Hybrid RAG** | Sparse retrieval plus dense/vector-space retrieval, merged by score fusion or Reciprocal Rank Fusion. | BM25 + TF-IDF vector-space baseline + weighted RRF. TF-IDF is not mislabeled as a semantic embedding model. | [DPR](https://arxiv.org/abs/2004.04906), [fusion analysis](https://arxiv.org/abs/2210.11934) |
...
|---|---|---|---|
| **CRAG** | Paper-specific. Generic relevance filtering should be called `crag-inspired` or corrective RAG. | Grade evidence, refine weak knowledge, and optionally fall back to another source. | Relevance filtering exists in advanced RAG; faithful CRAG is optional. [Paper](https://arxiv.org/abs/2401.15884) · [code](https://github.com/HuskyInSalt/CRAG) |
...
| **Adaptive RAG** | Umbrella rather than one algorithm. | Route no-retrieval, single-hop, or iterative retrieval by query complexity. | Decision framework and routing contract. |
...
The TAG database paradigm is more than embedding CSV rows. A database performs relational operations while an LM performs semantic operators that ordinary SQL cannot express. The runnable `TableStore` here demonstrates typed relational filtering, numeric aggregation, and row provenance—the deterministic database half of the architecture.
...
The lab implements user-scoped append and relevance-ranked recall. Source: [MemGPT](https://arxiv.org/abs/2310.08560)

### Tool-Augmented Generation

A model emits a typed request to an external capability and conditions later reasoning on the observation. Tool authority must come from the application, never from retrieved text. The lab implements an explicit allowlist and auditable arguments/results.
