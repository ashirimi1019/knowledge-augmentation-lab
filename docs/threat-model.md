...
| **TM-03 Metadata alias/TOCTOU** | **CTRL-03** composition-based recursive immutable snapshots, JSON-safe finite leaves, scalar normalization, exact built-in list/tuple handling, and detached general mappings | `test_document_normalizes_id_and_freezes_metadata`; `test_metadata_cannot_be_mutated_through_dict_base_class_descriptors`; `test_metadata_rejects_non_json_leaf_values`; `test_metadata_rejects_unsupported_container_implementations`; `test_document_and_trace_metadata_retain_general_mapping_compatibility` | No signed source provenance or ingestion attestation. |
...
| **TM-06 Tool argument escalation / snapshot divergence** | **CTRL-06** registered-name and keyword allowlists, callable signature validation, dedicated exact-built-in recursive tool-value freezing before invocation, rejection of behavior-bearing mappings/subclasses, immutable pre-call arguments and post-call outputs | `tests/test_tool_registry_security.py`; `test_property_tool_registry_rejects_unexpected_arguments`; `test_tool_registry_rejects_stateful_mapping_before_execution`; `test_tool_registry_rejects_custom_mapping_nested_in_plain_dictionary`; `test_tool_registry_snapshots_arguments_before_tool_execution`; `test_tool_registry_rejects_custom_mapping_output` | No semantic per-tool value schema, timeout, sandbox, caller authorization, rollback, or side-effect policy. Invalid output is detected only after the tool may have produced side effects. Concurrent mutation of otherwise supported exact containers is outside this in-process demonstration. |
...
| **TM-10 Supply-chain drift** | **CTRL-10** immutable Action SHAs, per-Python hashed development/application constraints, hashed build-tool constraints, no-isolation owned builds, strict package installs, CodeQL, Dependabot, protected `main` | `tests/test_repository_policy.py`; Python 3.10–3.13 CI jobs; live repository settings | Distribution metadata intentionally retains compatible ranges for downstream consumers; package indexes and the Python installer remain trusted infrastructure. |
