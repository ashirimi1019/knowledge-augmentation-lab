...
| **TM-06 Tool argument escalation** | **CTRL-06** registered-name allowlist, exact keyword-name allowlist, callable signature validation, finite JSON-compatible recursively immutable pre-call argument and post-call output snapshots, explicit replacement | `tests/test_tool_registry_security.py`; `test_property_tool_registry_rejects_unexpected_arguments`; `test_tool_registry_snapshots_arguments_before_tool_execution`; `test_tool_result_recursively_snapshots_output` | No semantic per-tool value schema, timeout, sandbox, caller authorization, or side-effect policy. Invalid output is detected only after the tool may have produced side effects. |
